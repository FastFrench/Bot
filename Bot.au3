...
$x_default = $BlueStacks_XY [0] + 52
$y_default = $BlueStacks_XY [1] + 93
...
Func CheckSearch()
   ...
   $GoldCheck = ""
   While $GoldCheck = ""
	  $GoldCheck = getGold($x_default, $y_default)
	  If $GoldCheck = "" Then
		 $GoldCheck = getGold($x_default+1, $y_default+1)
	  EndIf
	  Sleep(200)
	  If $RunState = False Then
		 ExitLoop
...
Func ReadValue()
   While 1
	  If $RunState = False Then
		 ExitLoop
	  EndIf
	  If Mod($SearchCount,20) = 0 Then
		 ClearBox()
	  EndIf
	  If getDarkElixir($x_default+1, $y_default+58) <> "" Or  getDarkElixir($x_default, $y_default+57) <> "" Then
		 $Gold = Number(getGold($x_default+1, $y_default+1))
		 If $Gold = "" Then
			$Gold = Number(getGold($x_default, $y_default))
		 EndIf
		 $Elixir = Number(getElixir($x_default+1, $y_default + 30))
		 If $Elixir = "" Then
			$Elixir = Number(getElixir($x_default, $y_default + 29))
		 EndIf
		 $Dark = Number(getDarkElixir($x_default+1, $y_default + 58))
		 If $Dark = "" Then
			$Dark = Number(getDarkElixir($x_default, $y_default + 57))
		 EndIf
		 $Trophy = Number(getTrophy($x_default+1, $y_default + 91))
		 If $Trophy = "" Then
			$Trophy = Number(getTrophy($x_default, $y_default + 90))
		 EndIf
	  ...
	  Else
		 ...
		 EndIf
		 $Dark = 0
		 $Trophy = Number(getTrophy($x_default+1, $y_default + 62))
		 If $Trophy = "" Then
			$Trophy = Number(getTrophy($x_default+1, $y_default + 60))
		 EndIf
		 If $Trophy = "" Then
			$Trophy = Number(getTrophy($x_default, $y_default + 61))
		 EndIf
		 If $Trophy = "" Then
			$Trophy = Number(getTrophy($x_default, $y_default + 59))
		 EndIf
		 GUICtrlSetData($Results, "(" & $SearchCount+1 & ") [G]: " & $Gold & Tab($Gold,12) & "[E]: " & $Elixir & Tab($Elixir,12) & "[D]: " & $Dark & Tab($Dark,12) & "[T]: " & $Trophy & @CRLF, -1)
	  EndIf
...
EndFunc
#ce
Func CheckScreenChange($a, $b, $c, $d)
...
Func MainScreen()
   If Not CheckScreenChange(0.25, 0.25, 0.75, 0.75) Then
	  ControlClick ($HWnD, "","", "left", "1", 293*$x_ratio, 329*$y_ratio)
	  ControlClick ($HWnD, "","", "left", "1", 400*$x_ratio, 408*$y_ratio)
	  GUICtrlSetData($Results, "Waiting for Clash of Clans to load..." & @CRLF, -1)
	  Sleep(6000)
   EndIf
   While Not CheckScreenChange(0.25, 0.25, 0.75, 0.75)
	  Sleep(1000)
   WEnd
   If Not CheckScreenChange(0.25, 0.25, 0.75, 0.75) Then
	  ControlClick ($HWnD, "","", "left", "1", 400*$x_ratio, 408*$y_ratio)
...
Func GoldChange()
   While 1
	  $Gold1 = Number(getGold($x_default, $y_default))
	  If $Gold1 < 100 Then
		 $Gold1 = Number(getGold($x_default, $y_default+1))
	  EndIf
	  Sleep(30000)
	  If $RunState = False Then
		 ExitLoop
	  EndIf
	  $Gold2 = Number(getGold($x_default, $y_default))
	  If $Gold2 < 100 Then
		 $Gold2 = Number(getGold($x_default, $y_default+1))
	  EndIf
	  If $Gold1 = $Gold2 Then
		 Return False
	  Else
